...
# Variables
rofi_theme="$HOME/.config/rofi/walpaper/selector.rasi"
...
adjusted_icon_size=$(echo "$icon_size" | awk '{if ($1 < 20) $1 = 20; if ($1 > 20) $1 = 30; print $1}')
...
modify_startup_config() {
  ...
  # **如果是符号链接，获取它指向的实际文件**
  if [[ -L "$startup_config" ]]; then
    startup_config=$(realpath "$startup_config")
  fi
